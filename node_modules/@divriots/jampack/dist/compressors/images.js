...
export async function compressImage(state, data, options) {
    var _a, _b;
    ...
    if (meta.format === 'svg') {
        if (!imageOptions.svg.optimization)
            return undefined;
        try {
    ...
    if (toFormat === 'unchanged') {
        switch (meta.format) {
            ...
            case 'jpg':
                ...
                break;
            case 'webp':
                sharpFile = sharpFile.webp(createWebpOptions(imageOptions.webp.options_lossly) || {});
                outputFormat = 'webp';
                break;
            case 'heif':
            case 'avif':
